...
<script>
  ...
  import { debounce } from '@byfrost/utils/function.js'
  import { createValidator } from 'lib/validation.js'
  import '../hdl_dashboard_item/index.svelte'
  ...
  const { joi } = window
  const limitStep = 20
  const limitMarkers = new Array(5).fill().map((_, index) => (index + 1) * limitStep)
    ...
    <div class="max-wrapper">
      ...
      <div class="max">
        <div class="markers hstretch"
          style="--gauge:{gauge}">
          {#each limitMarkers as marker (marker)}
