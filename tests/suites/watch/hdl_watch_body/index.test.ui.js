...
test('small screen display', async ({ page }) => {
  ...
  // under --small-screen (1025px): no size menu gutter, deeper hero margin
  await page.setViewportSize({ width: 900, height: 667 })
  await page.goto(getComponentUrl({ params: { layoutState: findModeState } }))

  await expect(page).toHaveScreenshot(getScreenshotPath('small-screen'))

  await page.setViewportSize({ width: 1280, height: 720 })

  // back over the breakpoint, onto the very rendering the find mode mount
  // settles on
  return expect(page).toHaveScreenshot(getScreenshotPath('find-mode'))
})
...
test.describe('on touch device', () => {
  ...
  test('mini screen display', async ({ page }) => {
    // few enough results for the manual add button to stand in frame, right
    // under a single row of posters
    const results = seekResults.slice(0, 2)
    ...
    await page.route(
      /themoviedb.*\/search\//,
      route => route.fulfill({ json: { results, total_results: results.length } })
    )
    ...
    // under --mini-screen (813px): the whole layout on a phone, the carousel
    // left without its navigation buttons
    await expect(page).toHaveScreenshot(getScreenshotPath('mini-screen'))
    ...
    // a device without hover enlarges the manual add label
    await expect(page).toHaveScreenshot(getScreenshotPath('mini-screen-add'))

    await page.setViewportSize({ width: 667, height: 375 })

    // a screen this short leaves the button just under the fold
    await scrollToGridEnd(page)

    // the same state on its side, where the label is held to a lesser size
    return expect(page).toHaveScreenshot(getScreenshotPath('mini-screen-add-landscape'))
  })

  test('carousel touch lock', async ({ page }) => {
    await page.route('/api/get-config/watch', route => route.fulfill({ json: config }))
    await page.route('/api/find-items', route => route.fulfill({ json: fixtureItems }))
    await page.route('/api/count-items', route => route.fulfill({ json: fixtureItems.length }))
    await page.route('/api/lookup-items', route => route.fulfill({ json: lookedUpItems }))
    ...
    // a sliver of grid at rest: the move is left to the view to scroll on
    await expect.poll(() => swipeCarousel(page)).toBe(false)

    // the grid stands some eighty pixels in sight, still short of the hundred
    // the carousel takes over at
    await scrollGrid(page, 60)
    await expect.poll(() => swipeCarousel(page)).toBe(false)

    // past the hundred, the carousel swallows the move instead
    await scrollGrid(page, 120)
    await expect.poll(() => swipeCarousel(page)).toBe(true)

    await scrollToGridTop(page)

    // the grid out of sight again, the move is released
    return expect.poll(() => swipeCarousel(page)).toBe(false)
  })
})
...
const scrollToGridTop = page => scrollGrid(page, 0)

const scrollGrid = (page, offset) => page.locator(tag)
  .evaluate((element, y) => element.scrollTo(0, y), offset)
...
// the move is kept under the carousel shift limit, so that no poster is
// switched along the way
const swipeCarousel = async page => {
  const carousel = page.locator('.hero-carousel')
  const start = { x: 180, y: 300 }

  await dispatchTouch(carousel, 'touchstart', start)

  const moveDefaultPrevented =
    await dispatchTouch(carousel, 'touchmove', { ...start, x: start.x - 30 })

  await dispatchTouch(carousel, 'touchend')

  return moveDefaultPrevented
}

// a generic Event carrying plain touch points is used because webkit does not
// expose the Touch constructor
