...
A working session here is a long running one, resumed a day or a week later rather than started
anew, so "once a day" means on the first run following a date change, which the harness announces
between two requests. When a check and a run go in the same command, join them with `&&` rather
than `;`, so a wrong build stops the run instead of merely annotating a wasted one.
